...
describe("codex app-server connection", () => {
  ...
  it("settles a write and exit race with terminal process details", async () => {
    const exited = deferred<CodexAppServerExitInfo>();
    const connection = createCodexAppServerConnection({
      command: process.execPath,
      args: [
    ...
    try {
      await expect(
        ...
        process.platform === "win32"
          ? /codex app-server stdin failed \(EOF\)/
          : /codex app-server exited \(code 7, signal null\)/,
      );
      await expect(exited.promise).resolves.toMatchObject(
        process.platform === "win32"
          ? {
              stderrTail: expect.stringContaining("stdin failed (EOF)"),
              spawnFailed: false,
            }
          : {
              code: 7,
    ...
    async () => {
      ...
      try {
        ...
        await expect(exited.promise).resolves.toMatchObject({
          code: null,
          signal: "SIGKILL",
          stderrTail: expect.stringMatching(/stdin failed \((?:EOF|EPIPE)\)/),
          spawnFailed: false,
        });
      } finally {
        await stopConnection(connection, exited.promise);
