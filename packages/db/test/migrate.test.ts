...
function dropRewindAddedTables(db: DbConnection): void {
  ...
  dropNativeSessionColumns(db);
  dropTypographyColumns(db);
...
function dropNativeSessionColumns(db: DbConnection): void {
  const columns = db.$client
    .prepare<[], TableInfoRow>("PRAGMA table_info(threads)")
    .all();
  if (columns.some((column) => column.name === "native_tail_fingerprint")) {
    db.$client
      .prepare("ALTER TABLE threads DROP COLUMN native_tail_fingerprint")
      .run();
  }
  if (columns.some((column) => column.name === "native_resume")) {
    db.$client.prepare("ALTER TABLE threads DROP COLUMN native_resume").run();
  }
}

function dropTypographyColumns(db: DbConnection): void {
  const columns = db.$client
    .prepare<[], TableInfoRow>("PRAGMA table_info(app_theme)")
    .all();
  if (columns.some((column) => column.name === "typography_profile")) {
    db.$client
      .prepare("ALTER TABLE app_theme DROP COLUMN typography_profile")
      .run();
  }
  if (columns.some((column) => column.name === "font_scale_percent")) {
    db.$client
      .prepare("ALTER TABLE app_theme DROP COLUMN font_scale_percent")
      .run();
  }
}
